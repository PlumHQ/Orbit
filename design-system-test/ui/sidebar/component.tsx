  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "bg-white text-gray-900 flex h-full w-60 flex-col py-2",
          className
        )}
        {...props}
      >
...
function SidebarFooter({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn("flex flex-col gap-2","fixed bottom-2 w-full", state === "expanded" ? "w-60" : "w-18", className)}
      {...props}
    />
  )
}
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-auto group-data-[collapsible=icon]:overflow-hidden bg-surface-background-beige-subtle",
        className
      )}
      {...props}
    />
...
function SidebarGroupLabel({    
  ...
  collapsedIllustration,
  className,
  asChild = false,
  ...props
}: React.ComponentProps<"div"> & { asChild?: boolean, expandedIllustration?: React.FC<IIconProps>, collapsedIllustration?: React.FC<IIconProps> }) {
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-sidebar-foreground/70 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md h-12 py-2 text-xs font-medium outline-hidden transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
      ...
      {...props as any}
    >
      {state === "expanded" && expandedIllustration ? (() => {
        const IconComponent =  expandedIllustration;
        return IconComponent ? <IconComponent /> : null;
      })() : collapsedIllustration ? (() => {
        const IconComponent =  collapsedIllustration;
        return IconComponent ? <IconComponent /> : null;
      })() : null}
    </Comp>
  )
  ...
  const button = (
    ...
    >
      <div className="flex flex-row items-center gap-2">
        ...
        {state === "expanded" ? <span className="text-base font-medium text-interactive-text-beige-subtle font-primary overflow-hidden text-ellipsis whitespace-nowrap">{item.title}</span> : null}
      </div>
      {item.children && state === "expanded" && <div className="flex flex-row items-center gap-2">
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "border-sidebar-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-2.5 py-0.5 ml-9",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
...
function SidebarMenuSubItem({
  ...
  return (
    <li
      ...
      className={cn("group/menu-sub-item relative  hover:bg-surface-background-beige-normal rounded-md", className)}
      {...props}
    />
  )
}
...
function SidebarMenuSubButton({
  ...
  return (
    state === "expanded" ? <div
      ...
      data-active={isActive}
      className={cn(
        "px-4 py-2 text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>svg]:text-sidebar-accent-foreground flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 outline-hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
    ...
    >
        <div className="flex flex-row items-center gap-2">
            <span className="text-base font-medium text-interactive-text-beige-subtle font-primary overflow-hidden text-ellipsis whitespace-nowrap">{item.title}</span>
        </div>
    </div> : null
  )
}
